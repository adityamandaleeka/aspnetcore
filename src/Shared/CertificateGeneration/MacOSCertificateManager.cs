...
internal sealed class MacOSCertificateManager : CertificateManager
{
    ...
    {
        if (IsTrusted(certificate)) // On macOS this check just ensures its on the system keychain
        {
            // A trusted certificate in macOS is installed into the system keychain and
...
{output}");
    ...
    private static void EnsureCertificateFolder() ///// todo: get rid of this. createdirectory handles the case where the dir exists anyway. Keeping it might be racy
    {
    ...
    {
        if (store.Name! != StoreName.My.ToString() || store.Location != store.Location) ///// what is this second clause supposed to be?
        {
    ...
    protected override void RemoveCertificateFromUserStoreCore(X509Certificate2 certificate)
    {
        ...
        {
            ///// wrong message?
            Log.MacOSAddCertificateToKeyChainError($@"There was an error importing the certificate into the user keychain '{certificate.Thumbprint}'.
